...
BasicGame.Preloader.prototype = {
	...
	preload: function () 
	{
		...
		theJSON = this.game.cache.getJSON('gamesJSON');

		for (var i = 0; i < theJSON.data.length; i++)
		{
			console.log("Adding: " + theJSON.data[i].name);

			// randomURL = "?" + Math.floor(Math.random() * 9999);
			randomURL = "";

			// if (theJSON.data[i].name == 'ads') continue;

			this.load.atlasJSONHash(
				theJSON.data[i].name,
				'assets/' + theJSON.data[i].name + '.png' + randomURL,
				'assets/' + theJSON.data[i].name + '.json' + randomURL,
				null);
		}

		this.load.image('transparency_bg','assets/transparency_bg.png');
		this.load.image('black_pixel','assets/black_pixel.png');
		this.load.image('red_pixel','assets/red_pixel.png');
		this.load.image('default_particle','assets/default_particle.png');
	},
